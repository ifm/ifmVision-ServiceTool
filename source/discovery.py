...
class DiscoveryClient(object):
    ...
    def __init__(self, interface, loggerBox):
    ...
    def detect_devices(self):
        if self.my_os == "Linux":
            ...
            if os.getuid() != 0:
                self.loggerBox.add_log_entry(msg="Check your privileges. "
                                                 "Root permissions are required for platform ".format(self.my_os))
                sys.exit(-1)
        self.loggerBox.add_log_entry(msg='{ip}: Sending UDP broadcast ...'.format(ip=self.interface))
        ...
        if self.my_os == "Linux":
            try:
                ...
                self.loggerBox.add_log_entry(msg="Socket setup failed. "
                                                 "Does the specified interface exist? Check using: ifconfig (in cmd)")
                sys.exit(1)
        elif self.my_os == "Windows":
            sock.bind((self.interface, self.port))
        else:
            self.loggerBox.add_log_entry(msg="Operation system {} not supported yet!".format(self.my_os))
        ...
        self.loggerBox.add_log_entry(msg="Receiving own broadcast msg: {msg} from: {server}"
                                     .format(msg=response, server=server))
        ...
        while True:
            try:
                ...
                if self.response_magic == struct.unpack('>I', response[0:4])[0]:
                    ...
                    self.loggerBox.add_log_entry(msg="{inf}: IFM device ({name}) found with IP {ip}"
